...
# Read encrypted OpenAI key + passphrase from env
# Use the same passphrase you used when running the helper encryption script.
...
# Initialize OpenAI client only if decryption worked
...
def debug():
    ...
    return jsonify({
        ...
        "units": u,
        "openai_client": "Available" if client else "Not Available (check encrypted key / decryption)",
...
def cli_chat():
    if not client:
        print("OpenAI API key missing or decryption failed. Check OPENAI_API_KEY_ENCRYPTED / OPENAI_PASSPHRASE")
        return
    context = ""
    try:
...
if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1] == "--cli":
        ...
        print("🌐 Running on http://127.0.0.1:5083")
        ...
        app.run(host="127.0.0.1", port=5083, debug=True, use_reloader=False)
